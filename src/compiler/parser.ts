...
module ts {
    ...
    module Parser {
        ...
        function indexOfParameterInParameterList(parameter: string, list?: ParameterDeclaration[]): number {
            if(list) {
                for (let i = 0; i < list.length; i++) {
                    if (list[i].name.kind === SyntaxKind.Identifier && (<Identifier>list[i].name).text === parameter) {
                        return i;                    
                    }
                }
            }
